...
public class ShotDatabaseHelper extends SQLiteOpenHelper {
	...
	private static final String TABLE_SHORTGAME="shortGame";
	...
	public static final String KEY_SHOT_TYPE="shotType";
	//public static final String KEY_SURFACE="surface";
	public static final String KEY_START_DISTANCE="startDistance";
	//public static final String KEY_GREEN="green";
	public static final String KEY_END_DISTANCE="endDistance";
	//public static final String KEY_DIRECTION="direction";
	//public static final String KEY_PUTTS = "putts";
	...
	public void onCreate(SQLiteDatabase db) {
		String CREATE_SHORTGAME_TABLE="CREATE TABLE "+TABLE_SHORTGAME+"("
				+ KEY_ID+" INTEGER PRIMARY KEY, "+KEY_SHOT_TYPE+" INTEGER,"
				+ KEY_SURFACE+" INTEGER, "+KEY_START_DISTANCE+" INTEGER,"
				+ KEY_GREEN+" INTEGER, "+KEY_END_DISTANCE+" INTEGER,"
				+KEY_DIRECTION+" INTEGER, "+KEY_PUTTS+" INTEGER"+")";
		db.execSQL(CREATE_SHORTGAME_TABLE);
		String CREATE_SHOTS_TABLE = "CREATE TABLE " + TABLE_SHOTS + "("
				...
				+ KEY_SHAPE + " INTEGER," + KEY_TEE + " INTEGER," + KEY_SURFACE
				+ " INTEGER," + KEY_COURSE + " TEXT," + KEY_HOLE + " INTEGER"
				+ ")";
		...
		String CREATE_ROUNDS_TABLE = "CREATE TABLE " + TABLE_ROUNDS + "("
				...
				+ " TEXT," + KEY_HOLES_PLAYED + " INTEGER," + KEY_NINE_PLAYED
				+ " INTEGER," + KEY_FRONT_SCORE + " INTEGER," + KEY_BACK_SCORE
				+ " INTEGER," + KEY_TOTAL_SCORE + " INTEGER," + KEY_FRONT_PUTTS
				+ " INTEGER," + KEY_FRONT_FAIRWAYS + " INTEGER,"
				+ KEY_FRONT_POSSIBLE_FAIRWAYS + " INTEGER," + KEY_FRONT_GREENS
				+ " INTEGER," + KEY_BACK_PUTTS + " INTEGER,"
				+ KEY_BACK_FAIRWAYS + " INTEGER," + KEY_BACK_POSSIBLE_FAIRWAYS
				+ " INTEGER," + KEY_BACK_GREENS + " INTEGER," + KEY_TOTAL_PUTTS
				+ " INTEGER," + KEY_TOTAL_FAIRWAYS + " INTEGER,"
				+ KEY_TOTAL_POSSIBLE_FAIRWAYS + " INTEGER," + KEY_TOTAL_GREENS
				+ " INTEGER" + ")";
		db.execSQL(CREATE_ROUNDS_TABLE);
		String CREATE_HOLES_TABLE = "CREATE TABLE " + TABLE_HOLES + "("
				+ KEY_ID + " INTEGER PRIMARY KEY, " + KEY_HOLE_NUMBER
				+ " INTEGER," + KEY_PAR + " INTEGER," + KEY_COURSE + " TEXT,"
				+ KEY_DATE + " TEXT," + KEY_FAIRWAY + " INTEGER," + KEY_GREEN
				+ " INTEGER," + KEY_PUTTS + " INTEGER," + KEY_TEE_CLUB
				+ " TEXT," + KEY_TEE_RESULT + " INTEGER," + KEY_SCORE
				+ " INTEGER" + ")";
	...
	public void onUpgrade(SQLiteDatabase db, int oldVerson, int newVersion) {
		...
		db.execSQL("DROP TABLE IF EXISTS " + TABLE_SHORTGAME);
		onCreate(db);

	}
	
	public void addShortGameShot(ShortGameShot s){
		SQLiteDatabase db = this.getWritableDatabase();
		ContentValues values = new ContentValues();
		
		
		values.put(KEY_SHOT_TYPE, s.getShotType());
		values.put(KEY_SURFACE, s.getStartLocation());
		values.put(KEY_START_DISTANCE, s.getOrigDistRange());
		values.put(KEY_GREEN,s.isHitGreen()?1:0);
		values.put(KEY_END_DISTANCE, s.getFinalDistRange());
		values.put(KEY_DIRECTION, s.getMissDirection());
		values.put(KEY_PUTTS,s.getPuttsAfter());
		
		
		db.insert(TABLE_SHORTGAME,null,values);
		db.close();
	}
	...
	public ArrayList<FullShot> getTeeShotsOnHole(String course, int hole) {
		String selectQuery = "SELECT * FROM " + TABLE_SHOTS + " WHERE "
				+ KEY_TEE + "=1 AND " + KEY_COURSE + " ='" + course + "' AND "
				+ KEY_HOLE_NUMBER + "=" + hole;
		return getAllShotsByQuery(selectQuery);
	}
	...
	private ArrayList<FullShot> getAllShotsByQuery(String selectQuery) {
		...
		if (cursor.moveToFirst()) {
			do {
				...
				FullShot s = new FullShot(club, course, hole, tee, direction,
						shape, distance, surface);
				shotList.add(s);
	...
	public ArrayList<ShortGameShot> getAllShortGameShots() {
		String selectQuery = "SELECT * FROM " + TABLE_SHORTGAME;
		return getAllShortGameShotsByQuery(selectQuery);
	}
	public ArrayList<ShortGameShot> getAllBunkerShots(){
		String selectQuery = "SELECT * FROM "+ TABLE_SHORTGAME+" WHERE "+KEY_SHOT_TYPE+"="+ShortGameShot.BUNKER_SHOT;
		return getAllShortGameShotsByQuery(selectQuery);
	}
	private ArrayList<ShortGameShot> getAllShortGameShotsByQuery(String selectQuery){
		ArrayList<ShortGameShot> shotList = new ArrayList<ShortGameShot>();
		SQLiteDatabase db = this.getReadableDatabase();
		Cursor cursor = db.rawQuery(selectQuery, null);
		
		if(cursor.moveToFirst()){
			do{
				int shotType = Integer.parseInt(cursor.getString(1));
				int startLoc = Integer.parseInt(cursor.getString(2));
				int origDistRange = Integer.parseInt(cursor.getString(3));
				boolean green = Integer.parseInt(cursor.getString(4))==1;
				int finalDistRange = Integer.parseInt(cursor.getString(5));
				int missDir = Integer.parseInt(cursor.getString(6));
				int puttsAfter = Integer.parseInt(cursor.getString(7));
				ShortGameShot sgs = new ShortGameShot(null, null, 0,
						shotType, origDistRange, finalDistRange,
						green, missDir, startLoc);
				sgs.setPuttsAfter(puttsAfter);
				shotList.add(sgs);
			}while(cursor.moveToNext());
		}
		db.close();
	...
	public ArrayList<Hole> getAllHolesByCourse(String course) {
		String query = "SELECT * FROM " + TABLE_HOLES + " WHERE " + KEY_COURSE
				+ "='" + course + "'";
	...
	public ArrayList<Hole> getAllHolesByHole(String course, int num) {
		String query = "SELECT * FROM " + TABLE_HOLES + " WHERE " + KEY_COURSE
				+ "='" + course + "' AND " + KEY_HOLE_NUMBER + "=" + num;
		return getAllHolesByQuery(query);
	}
	...
	public ArrayList<RoundSQL> getAllRoundsOnCourse(String course) {
		String selectQuery = "SELECT * FROM " + TABLE_ROUNDS + " WHERE "
				+ KEY_COURSE_NAME + "=" + course;
		return getAllRoundsByQuery(selectQuery);
	}
	...
	private ArrayList<RoundSQL> getAllRoundsByQuery(String query) {
		...
		if (cursor.moveToFirst()) {
			do {
				...
				r.totalPossibleFairways = Integer
